...
<footer class="text-muted text-center" style="display:none">
   <p>© 2021 - '.$year.' '.$site_name.' - Design By: <span id="credits"><a class="credits_a" href="https://gresscode.com" target="_blank">gresscode.com</a></span></p>
</footer>
<!-- ///////////// Js Files ////////////////////  -->
<!-- Jquery -->
...
if($mod =='history' OR $mod=='cuty' OR $mod=='sppd' OR $mod=='izin' OR $mod=='job'){
echo'
...
<script type="text/javascript">
	...
	$(document).on('click', '.btn-modal', function(){
		...
		var koordinat = $('.latitude').html();
		var explode = koordinat.split(",");
		
		var latitude  = explode[0];
		var longitude = explode[1];

		var name = $('.latitude').html();
		$(".modal-title-name").html(name);
		document.getElementById("iframe-map").innerHTML ='<iframe src="sw-mod/maps.php?latitude='+latitude+'&longitude='+longitude+'&name='+name+'" frameborder="0" width="100%" height="400px" marginwidth="0" marginheight="0" scrolling="no">';
	});
</script>
<?php 
...
<script type="text/javascript">
    ...
    $(document).on('change','#webcam',function(){
        ...
        $.ajax({
          ...
          url:'./sw-proses?action=absent&latitude='+latitude+'&shift_id='+shift_id+'&building_id='+building_id+'',
          method:'POST',
          data:form_data,
          contentType:false,
          cache:false,
          processData:false,
		  beforeSend: function () {
				if ($('[name="shift_id"]').val() == '') {
					//alert("Shift Harus Diisi");
					swal({title: 'Oops!', text: 'Shift harus diisi', icon: 'error', timer: 3500,});
					return false;
				}
			},
...
<script type="text/javascript">
	...
	var building_id = $('#building_id').val();
        ...
        Webcam.snap( function(data_uri) {
            ...
            Webcam.upload(data_uri, './sw-proses?action=absent&latitude='+latitude+'&shift_id='+shift_id+'&building_id='+building_id+'',
                function(code,text) {
                    $data       =''+text+'';
                    var results = $data.split("/");
                    $results = results[0];
