...
<table id="swdatatable" class="table table-bordered">
  ...
  <tbody>';
  $query="SELECT employees.employees_code,employees.employees_name,cuty.* FROM employees,cuty WHERE employees.id=cuty.employees_id order by cuty.cuty_id DESC";
 ...
 while ($row= $result->fetch_assoc()) {
    if($row['cuty_status'] =='1'){
      ...
      $status='<span class="text-muted">Blm.Diapprove</span>';
    }
    $no++;
    echo'
    <tr>
