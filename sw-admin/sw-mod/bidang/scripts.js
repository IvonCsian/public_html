...
$('.add-bidang').submit(function (e) {
    if($('#nama').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"sw-mod/bidang/proses.php?action=add",
            type: "POST",
...
/* -------------------- Edit ------------------- */
$('.update-bidang').submit(function (e) {
    if($("#title").val()==""){    
         swal({title: 'Oops!', text: 'Harap bidang inputan tidak boleh ada yang kosong.!', icon: 'error', timer: 1500,});
         loading();
        return false;
        ...
        e.preventDefault();
        $.ajax({
            url:"sw-mod/bidang/proses.php?action=update",
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
        ...
        var id = $(this).attr("data-id");
          ...
          .then((value) => {
            if(value) {
                loading();
                $.ajax({  
                     url:"sw-mod/bidang/proses.php?action=delete",
                     type:'POST',    
                     data:{id:id},  
                    success:function(data){ 
                        if (data == 'success') {
