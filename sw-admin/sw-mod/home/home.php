...
<section class="content">
      ...
      <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
          ...
          <table class="table">
            <tbody>
				...
				if($row_cuty['cuty_status'] == "3"){
					$status = '<label class="label label-warning">Blm.Approve</label>';
				}
				else if($row_cuty['cuty_status'] == "1"){
					$status = '<label class="label label-info">Disetujui</label>';
				}
				else{
					$status = '<label class="label label-danger">Tdk.Disetujui</label>';
				}
                $no++;
                  echo'
                  <td class="text-center">'.$no.'</td>
                  <td>'.$row_cuty['employees_name'].'</td>
                  <td>'.tgl_ind($row_cuty['cuty_start']).' s.d. '.tgl_ind($row_cuty['cuty_end']).'</td>
	   ...
	   <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
        <div class="box box-solid">
        <div class="box-header with-border">
          <h3 class="box-title">Permohonan SPPD</h3>
          <div class="box-tools pull-right">
            <a href="sppd" class="btn btn-info btn-flat">Data SPPD</a>
          </div>
        </div>
          <div class="box-body no-padding">
            <table class="table">
            <tbody>
                ...
                <tr>';
                $query_cuty="SELECT employees.employees_code,employees.employees_name,sppd.* FROM sppd LEFT JOIN employees ON employees.id = sppd.employees_id WHERE sppd.is_del = 0 and status_sppd = 1 ORDER BY sppd.id_sppd DESC LIMIT 10";
                $result_cuty = $connection->query($query_cuty);
                if($result_cuty->num_rows > 0){
                $no=0;
                while ($row_cuty= $result_cuty->fetch_assoc()) {
				
				if($row_cuty['status_sppd'] == "1"){
					$status = '<label class="label label-warning">Blm.Approve</label>';
				}
				else if($row_cuty['status_sppd'] == "2"){
					$status = '<label class="label label-info">Disetujui</label>';
				}
				else{
					$status = '<label class="label label-danger">Tdk.Disetujui</label>';
                ...
                $no++;
                  echo'
                  <td class="text-center">'.$no.'</td>
				   <td>'.$row_cuty['employees_code'].'</td>
                  <td>'.$row_cuty['employees_name'].'</td>
                  <td>'.tgl_ind($row_cuty['tgl_awal']).' s.d. '.tgl_ind($row_cuty['tgl_akhir']).'</td>
                  <td class="text-right">'.$status.'</td>
                </tr>';}
                }
          echo'
