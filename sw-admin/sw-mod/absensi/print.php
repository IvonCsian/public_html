...
echo'
    <section class="container_box">
	  ...
	  */
	  ...
	  $query_absen ="SELECT presence_id,presence_date,shift_time_in,shift_time_out,time_in,time_out,picture_in,picture_out,present_id,latitude_longtitude_in,information,TIMEDIFF(TIME(time_in),shift_time_in) AS selisih,if (time_in>shift_time_in,'Telat',if(time_in='00:00:00','Tidak Masuk','Tepat Waktu')) AS status,TIMEDIFF(TIME(time_out),shift_time_out) AS selisih_out FROM presence WHERE $filter ORDER BY presence_id DESC";
      $result_absen = $connection->query($query_absen);
      $row_absen = $result_absen->fetch_assoc();
	  
	  if($row_absen['shift_time_in'] == ""){
		$shift_time_in = $row_shift['time_in']; 
	  }
	  else{
		$shift_time_in = $row_absen['shift_time_in'];
	  }
	  
	  if($row_absen['shift_time_out'] == ""){
		 $shift_time_out = $row_shift['time_out'];
	  }
	  else{
		$shift_time_out = $row_absen['shift_time_out'];
	  }
...
echo'
    <section class="container_box">
      ...
      $query_absen ="SELECT presence_id,presence_date,shift_time_in,shift_time_out,time_in,time_out,picture_in,picture_out,present_id,latitude_longtitude_in,information,TIMEDIFF(TIME(time_in),shift_time_in) AS selisih,if (time_in>shift_time_in,'Telat',if(time_in='00:00:00','Tidak Masuk','Tepat Waktu')) AS status FROM presence WHERE $filter";
      $result_absen = $connection->query($query_absen);
      $row_absen = $result_absen->fetch_assoc();
