...
<div class="box-body">
          ...
          <table id="swdatatable" class="table table-bordered">
            ...
            <tbody>';
            $query="SELECT user.user_id,user.username,user.fullname,user.email,user.registered,user.level,user.level,user_level.level_id,user_level.level_name,user.kd_status FROM user,user_level WHERE user.level=user_level.level_id order by user.user_id DESC";
           ...
           while ($row_a= $result->fetch_assoc()) {
              ...
              <tr>
                ...
                <td>'.$row_a['kd_status'].'</td>
                <td class="text-right">
                  <div class="btn-group btn-group-xs">';
                  if($level_user==1){
                  echo'
                  <a href="#modalEdit" class="btn btn-warning btn-xs enable-tooltip" title="Edit" data-toggle="modal"';?> onclick="getElementById('txtid').value='<?PHP echo $row_a['user_id'];?>';getElementById('txtnama').value='<?PHP echo $row_a['fullname'];?>';getElementById('txtuser').value='<?PHP echo $row_a['username'];?>';getElementById('txtemail').value='<?PHP echo $row_a['email'];?>';getElementById('txtlevel').value='<?PHP echo $row_a['level'];?>';getElementById('txtstatus_peg').value='<?PHP echo $row_a['kd_status'];?>';"><i class="fa fa-pencil-square-o"></i> Ubah</a><?PHP }
                  else{
                      // cek level 2 berdasarkan id login
                      if($user_id ==$SESSION_ID){
                       echo'<a href="#modalEdit" class="btn btn-warning btn-xs enable-tooltip" title="Edit" data-toggle="modal"';?> onclick="getElementById('txtid').value='<?PHP echo $row_a['user_id'];?>';getElementById('txtnama').value='<?PHP echo $row_a['fullname'];?>';getElementById('txtuser').value='<?PHP echo $row_a['username'];?>';getElementById('txtemail').value='<?PHP echo $row_a['email'];?>';getElementById('txtlevel').value='<?PHP echo $row_a['level'];?>';getElementById('txtstatus_peg').value='<?PHP echo $row_a['kd_status'];?>;"><i class="fa fa-pencil-square-o"></i> Ubah</a><?PHP
                      }else{
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        if($kd_status ==""){
          echo'
          <label>Status Pegawai</label>
           <select class="form-control" name="status_peg" id="txtstatus_peg">
            <option value="">- Pilih -</option>';
            $query="SELECT * from status_pegawai order by id_status ASC";
            $result = $connection->query($query);
            while($row = $result->fetch_assoc()) { 
            ...
            echo'<input type="hidden" name="status_peg" id="txtstatus_peg">';
          }
        echo'
        </div>
