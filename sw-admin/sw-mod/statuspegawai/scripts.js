...
$('.add-status').submit(function (e) {
    if($('#nama').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"sw-mod/statuspegawai/proses.php?action=add",
            type: "POST",
            ...
            success: function (data) {
                if (data == 'success') {
                    swal({title: 'Berhasil!', text: 'Bidang berhasil disimpan.!', icon: 'success', timer: 1500,});
                   $('#modalAdd').modal('hide');
...
$('.update-status').submit(function (e) {
    if($("#title").val()==""){    
        ...
        e.preventDefault();
        $.ajax({
            url:"sw-mod/statuspegawai/proses.php?action=update",
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
        ...
        var id = $(this).attr("data-id");
          ...
          .then((value) => {
            if(value) {
                loading();
                $.ajax({  
                     url:"sw-mod/statuspegawai/proses.php?action=delete",
                     type:'POST',    
                     data:{id:id},  
                    success:function(data){ 
                        if (data == 'success') {
